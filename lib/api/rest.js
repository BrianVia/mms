...
restRouter.get('/folders', (req, res) => {
	...
	if ((lpath === '' || lpath === '/') && process.platform == 'win32') {
		...
		windowsFS
			...
			.catch(err => {
				logger.warn(
					'Problem reading content of ' + lpath + ' (' + err.message + ')'
				);
			});
		return;
	}
	fs.readdir(lpath, (err, items) => {
		...
		var folders = items.filter(file => {
			try {
				...
				// In order to ignore Windows locked files (try cause errors in statSync()).
				return undefined;
			}
		});
		if (lpath !== '/') folders.splice(0, 0, '..');
		res.json(folders);
	});
});
...
restRouter.get('/tracks/:collectionId', (req, res) => {
	...
	if (req.params.collectionId) {
		...
		for (var c of collections) {
			if (c.id == req.params.collectionId) params.folders = c.folders;
		}
	}
	if (req.query.sort) params.sort = req.query.sort;
	if (req.query.filter) params.filters = JSON.parse(req.query.filter);
...
restRouter.get('/playlists/:guid', (req, res) => {
	...
	// root
		parent_guid = null;
	MediaProvider.registry.getPlaylistContent(
		parent_guid,
		(err, playlists, tracks) =>
			res.json({
				playlists: playlists,
				tracks: tracks
			})
...
restRouter.get('/playlists', (req, res) => {
	MediaProvider.registry.getAllPlaylists(
		(err, playlists, tracks) => {
